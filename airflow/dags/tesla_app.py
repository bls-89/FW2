...
#--------------
    ...
    cursor.execute(f"""CREATE TEMP TABLE IF NOT EXISTS tesla_raw (
    ...
    #cursor.execute("TRUNCATE TABLE tesla_raw") #очистка RAW таблицы когда она не временная

    for datetime, values in data['Time Series (15min)'].items():
        cursor.execute(f"""
            INSERT INTO tesla_raw (DateTime, Open, High, Low, Close, Volume)
    ...
    cursor.execute(f"""CREATE TABLE IF NOT EXISTS tesla_daily (
        DateTime TIMESTAMP(0),
        Open FLOAT,
        High FLOAT,
        Low FLOAT,
    ...
    conn.commit()
    cursor.execute("TRUNCATE TABLE tesla_daily")
    conn.commit()

    #выбираем из сырых данных те которые отвечают нашим требованиям и записываем в чистую таблицу
    cursor.execute(f"""INSERT INTO tesla_daily
    SELECT *
    FROM tesla_raw
    WHERE date_trunc('day', DateTime) = CURRENT_DATE - INTERVAL '1 day';;""")
    conn.commit()
    cursor.execute("SELECT * FROM tesla_daily")
    res = cursor.fetchall()

    print(res)
    #добавляем все в архивную таблицу
    cursor.execute(f"""INSERT INTO tesla_full
    SELECT * FROM tesla_daily;""")
    conn.commit()
    cursor.close()
    conn.close()
