...
#--------------
    ...
    cursor.execute(f"""CREATE TEMP TABLE IF NOT EXISTS ibm_raw (
    ...
    #cursor.execute("TRUNCATE TABLE ibm_raw") #очистка RAW таблицы когда она не временная

    for datetime, values in data['Time Series (15min)'].items():
        cursor.execute(f"""
            INSERT INTO ibm_raw (DateTime, Open, High, Low, Close, Volume)
    ...
    cursor.execute(f"""CREATE TABLE IF NOT EXISTS ibm_daily (
        DateTime TIMESTAMP(0),
        Open FLOAT,
        High FLOAT,
        Low FLOAT,
